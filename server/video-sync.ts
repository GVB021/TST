...
const textControllerSessions = new Map<string, Set<string>>(); // sessionId -> Set of controller userIds
...
async function getWsIdentity(sessionId: string, req: any) {
  ...
  return { userId, role: studioRole, name };
}
  ...
  wss.on("connection", (ws: WebSocket & { userId?: string; role?: string; name?: string; sessionId?: string }, req) => {
    ...
    ws.on("message", (data) => {
      try {
        ...
        } else if (msg.type === "video-seek" && typeof msg.lineIndex === "number") {
          if (!isPrivileged && !isController) return;
        } else if (!canControl) {
